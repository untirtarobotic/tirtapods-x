...
bool detectLine () {
  if (line::isDetected && CounterRead == 0) {
    ...
    while ((currentFakeCounter - startFakeCounter) < 17000) {
      lcd::message(1, lcd::ROTATING_CW);
      currentFakeCounter = millis();
      legs::rotateCW();
    } 
  ...
  if (line::isDetected && CounterRead == 3) {
    ...
    while ((currentCounter - startCounter) < 17800) {
      lcd::message(1, lcd::MOVING_FORWARD);
      currentCounter = millis();
      legs::forward();
    }
    ...
    while ((currentCounter - startCounter) < 20800) {
      lcd::message(1, lcd::MOVING_FORWARD);
      currentCounter = millis();
      legs::forward();
    }
  ...
  if (line::isDetected && CounterRead == 4) {
    ...
    while ((currentFakeCounter - startFakeCounter) < 14000) {
      lcd::message(1, lcd::SHIFTING_LEFT);
      currentFakeCounter = millis();
      legs::shiftLeft();
    }
