...
bool avoid3Ladder (bool inverse = false) {
  if (proxy::isDetectingSomething && CurrentState == 0) {
    ...
    if (inverse) {
      ...
      while ((currentCounter - startCounter) <= (20500)) {
        legs::forward();
        currentCounter = millis();
      }
    }
...
bool detectLine() {
  ...
  if (line::isDetected && CounterRead == 0) {
    ...
    while ((currentFakeCounter - startFakeCounter) < 7300) {
      lcd::message(1, lcd::ROTATING_CCW);
      currentFakeCounter = millis();
      legs::rotateCCW();
    }
  ...
  if (line::isDetected && CounterRead == 5) {
    ...
    while ((currentFakeCounter - startFakeCounter) < 6300) {
      lcd::message(1, lcd::ROTATING_CCW);
      currentFakeCounter = millis();
      legs::rotateCCW();
    }
  ...
  if(line::isDetectedGlue && CounterRead == 4){
    ...
    while ((currentCounter - startCounter) < 6700){
      lcd::message(1, lcd::ROTATING_CCW);
      currentCounter = millis();
      legs::rotateCCW();
    }
    while ((currentCounter - startCounter) < 12400) {
      lcd::message(1, lcd::MOVING_FORWARD);
      currentCounter = millis();
      legs::forward();
    }
